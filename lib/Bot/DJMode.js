...
var joinCmd = function(user, bot, dj, msg, args) {
    ...
    if(r == -1) {
        dj.handler.sendPrivateMessage(user, "You already are in the wait list.");
    } else if(r == -2) {
        dj.handler.sendPrivateMessage(user, "The wait list is full.");
    } else {
        dj.handler.sendPrivateMessage(user, "You joined the wait list! Use `song [url]` to set your song");
    }
};

var youtubeCmd = function(user, bot, dj, msg, args) {
    if(args.length < 2) {
        dj.handler.sendPrivateMessage(user, "/" + args[0] + " `url`");
        return;
    }
    ...
    try {
        ...
        dj.handler.sendPrivateMessage(user, "Invalid Song URL");
        add = false;
    }
    if(add) {
        dj.addToQueue(yt);
...
class DJMode extends Mode {
    ...
    init(dj) {
        super.init(dj);
        this.waitlist = new WaitList(dj.bot, dj, this.opt['limit'] || 50, this.opt['dj-role'], this.opt['list-role']);
